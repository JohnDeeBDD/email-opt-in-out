<?php
/**
 * Rendering for the public unsubscribe page (PRD Sections 23, 24, 36).
 *
 ...
 *
 * Opt-in is silent and renders nothing at all, so nothing here speaks for it.
 *
...
class ActionPageView {
		...
		self::layout(
			...
			function () use ( $action, $email, $action_code, $title, $intro, $button ) {
				...
				<form method="post" action="<?php echo \esc_url( Urls::action_url( $action, $action_code ) ); ?>" class="aiplugin5055-form" data-aiplugin5055-action-form>
					...
					<button type="submit" class="aiplugin5055-button aiplugin5055-button--danger">
	...
	/**
	 * Confirmation shown once the unsubscribe has been recorded.
	 *
	 * @param string $action   opt_out.
	 * @param array  $campaign Campaign record.
	 * @param string $email    Decoded address.
