 ...
 *
 * These functions are the definition of the encoding, not a copy of it. They
 * are pure — no WordPress, no filesystem, no clock, no randomness — so both
 * the plugin and the campaign manager can call them and no second
...
final class EmailCodec {
	 ...
	 *
	 * A code generated from a differently-cased address decodes to a
	 * differently-cased address, and the site would look up the wrong user, so
	 * every entry point canonicalizes before it does anything else.
	 *
	 ...
	 *
	 * Bytes are consumed MSB-first into a bit accumulator and emitted five bits
	 * at a time. A trailing partial group is left-shifted into place rather
	 * than padded with a pad character, which is why the output carries no `=`
	 * and why its length is `ceil( strlen( $canonical_email ) * 8 / 5 )`.
	 *
